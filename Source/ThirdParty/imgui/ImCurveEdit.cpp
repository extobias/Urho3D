...
namespace ImCurveEdit
{
   ...
   static ImVec4 ScreenToWorld(const ImVec2& screenPos, const glm::mat4& projection, const glm::mat4& view, const ImVec2& viewSize)
   {
       ...
       float x = sp2.x * 2.0f - 1.0f;
       float y = (1.0f - sp2.y) * 2.0f - 1.0f;

       glm::vec4 near(x, y, 0.0f, 1.0f);
       glm::vec4 far(x, y, 1.0f, 1.0f);

       glm::mat4 viewProjInv = glm::inverse(view * projection);

       glm::vec3 origin = viewProjInv * near;
       glm::vec3 direction = glm::normalize(viewProjInv * far);

       // glm::vec3 viewSpaceDir = (view * glm::vec4(direction, 0.0f));
       // float rayDistance = (Max(screenPos.z_ - GetNearClip(), 0.0f) / viewSpaceDir.z_);

       // IMGUI_DEBUG_LOG("sp <%f, %f> screenPos <%f, %f> viewSize <%f, %f>\n", sp2.x, sp2.y, screenPos.x, screenPos.y, viewSize.x, viewSize.y);
...
//   }
   ...
   {
       glm::mat4 objectTrans(*(glm::mat4*)transform);
       glm::vec3 objectPos(objectTrans[3]);

       glm::vec4 pos(worldPos.x + objectPos.x, worldPos.y + objectPos.y, 0.0f, 1.0f);
       glm::vec4 trans = viewProjection * pos;
   ...
   static int DrawPoint(ImDrawList* draw_list, ImVec2 pos, const ImVec2 size, const ImVec2 offset, bool edited)
   {
      ...
      for (int i = 0; i < 4; i++)
      {
         // offsets[i] = pos * size + localOffsets[i]* pointSize + offset;
          offsets[i] = pos + localOffsets[i] * pointSize;
      }
...
//         draw_list->AddPolyline(offsets, 4, 0xFF0080FF, true, 2.0f);
      ...
      if (edited)
         draw_list->AddPolyline(offsets, 4, 0xFFFF0000, true, 3.0f);
      else if (ret)
          draw_list->AddPolyline(offsets, 4, 0xFF00FF00, true, 2.0f);
      else
         draw_list->AddPolyline(offsets, 4, 0xFF0000FF, true, 2.0f);
   ...
   int Edit(Delegate &delegate, const ImVec2& size, unsigned int id, const ImRect *clippingRect, ImVector<EditPoint> *selectedPoints)
   {
      ...
      // es la linea que aparece abajo
      draw_list->AddLine(ImVec2(-1.f, -min.y/range.y) * viewSize + offset, ImVec2(1.f, -min.y / range.y) * viewSize + offset, 0xFFFF0000, 1.5f);
...
//             }
          ...
          for (size_t p = 0; p < ptCount; p++)
          {
             ImVec2 worldPos = WorldToScreen(pts[p], transform, viewProjection, size);
       ...
       if (overSelectedPoint && io.MouseDown[0])
       {
           if ((fabsf(io.MouseDelta.x) > 0.f || fabsf(io.MouseDelta.y) > 0.f) && !selection.empty())
           {
               ...
               {
                   const ImVec2* pts = delegate.GetPoints(sel.curveIndex);
                   const ImVec2 p = originalPoints[originalIndex];// + (io.MousePos - mousePosOrigin) * sizeOfPixel;
                   ...
                   const int newIndex = delegate.EditPoint(sel.curveIndex, sel.pointIndex, p + mouseDiff);
                   if (newIndex != sel.pointIndex)
                   {
                       selection.erase(sel);
                       selection.insert({ sel.curveIndex, newIndex });
