...
public:
    ...
    virtual bool SetText(const char *text) override { return m_header.SetText(text); }
    virtual bool GetText(TBStr &text) override { return m_header.GetText(text); }
	using TBWidget::GetText; ///< Make all versions in base class available.

    virtual void SetValue(int value) override;
    virtual int GetValue() override { return m_toggle_container.GetValue(); }

    virtual TBWidget *GetContentRoot() override { return m_toggle_container.GetContentRoot(); }
